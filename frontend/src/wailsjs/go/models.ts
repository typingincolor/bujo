...
export namespace service {
	...
	export class MultiDayAgenda {
	    Overdue: domain.Entry[];
	    Days: DayEntries[];
	
	    static createFrom(source: any = {}) {
	        return new MultiDayAgenda(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.Overdue = this.convertValues(source["Overdue"], domain.Entry);
	        this.Days = this.convertValues(source["Days"], DayEntries);
	    }
