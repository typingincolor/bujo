...
describe('Bug #3 & #4: Popover Display Issues', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    localStorage.clear()
  })

  it('Bug #3: popover does not overlap migrate entry box in weekly review', async () => {
    const weekData = createMockAgenda({
      Days: [createMockDayEntries({
        Date: new Date().toISOString().split('T')[0] + 'T00:00:00Z',
        Entries: [
          createMockEntry({ ID: 1, Type: 'Task', Content: 'High priority task', Priority: 'high' }),
        ],
      })],
    })
    ...
    // Click attention item to open popover
    await userEvent.click(screen.getByText('High priority task'))

    await waitFor(() => {
      expect(screen.getByTestId('entry-context-popover')).toBeInTheDocument()
    })

    // Verify popover has proper positioning attributes
    const popover = screen.getByTestId('entry-context-popover')

    // Popover should have collision padding to avoid overlapping bottom elements
    // This is handled by Radix UI's collisionPadding prop
    expect(popover).toHaveAttribute('data-side')

    // Popover should be positioned above the bottom capture bar
    const computedStyle = window.getComputedStyle(popover)
    const zIndex = parseInt(computedStyle.zIndex)
    expect(zIndex).toBeGreaterThan(40) // Higher than capture bar z-index
  })

  it('Bug #4: clicking meetings triggers popover display', async () => {
    const weekData = createMockAgenda({
      Days: [createMockDayEntries({
        Date: new Date().toISOString().split('T')[0] + 'T00:00:00Z',
        Entries: [
          ...
          createMockEntry({ ID: 2, Type: 'Note', Content: 'Discussed feature X', ParentID: 1 }),
          createMockEntry({ ID: 3, Type: 'Task', Content: 'Follow up on Y', ParentID: 1 }),
        ],
      })],
    })
    ...
    // Find and click the meeting in the meetings section
    const meetingsSection = screen.getByTestId('week-summary-meetings')
    const meetingItem = within(meetingsSection).getByText('Team standup')
    ...
    // Should show meeting content and children in the popover
    const entryTree = within(popover!).getByTestId('entry-tree')
...
describe('Bug #8 & #9: Navigation Issues', () => {
  ...
  it('Bug #9: back button works in journal view after navigation from popover', async () => {
    const weekData = createMockAgenda({
      Days: [
        createMockDayEntries({
          Date: new Date().toISOString().split('T')[0] + 'T00:00:00Z',
          Entries: [
            createMockEntry({ ID: 1, Type: 'Task', Content: 'Attention task', Priority: 'high' }),
          ],
        }),
      ],
    })
    ...
    await userEvent.click(screen.getByText('Attention task'))

    await waitFor(() => {
      expect(screen.getByTestId('entry-context-popover')).toBeInTheDocument()
    })

    // Click "Go to entry" to navigate to journal view
    await userEvent.click(screen.getByText(/go to entry/i))

    await waitFor(() => {
      // Should be in today/journal view
      expect(screen.getByTestId('capture-bar')).toBeInTheDocument()
    })
    ...
    // Should return to week view
    await waitFor(() => {
      expect(screen.getByTestId('week-summary')).toBeInTheDocument()
    })

    // Back button should still be visible (multi-level navigation)
    const backButtonAgain = screen.getByRole('button', { name: /back/i })
    expect(backButtonAgain).toBeInTheDocument()

    // Click back again should return to original journal view
    ...
    // Now back button should disappear (at the start of history)
