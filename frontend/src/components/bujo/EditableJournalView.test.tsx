...
describe('EditableJournalView', () => {
  ...
  describe('deletion handling', () => {
    it('shows deletion count when deletions exist', () => {
      mockUseEditableDocument.mockReturnValue(
        createMockState({
          deletedEntries: [
            { entityId: 'entity-1', content: '. Deleted task' },
            { entityId: 'entity-2', content: '- Deleted note' },
          ],
        })
      )
      ...
      expect(screen.getByText(/2 deletions pending/i)).toBeInTheDocument()
    })

    it('opens deletion review dialog when saving with deletions', async () => {
      mockUseEditableDocument.mockReturnValue(
        createMockState({
          isDirty: true,
