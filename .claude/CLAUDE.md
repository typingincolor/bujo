...
Most entities use event sourcing — every mutation creates a new versioned row. The following tables use event sourcing: `habits`, `habit_logs`, `lists`, `list_items`, `day_context`, `goals`.
...
Repository mutations for event-sourced tables MUST follow this pattern:
...
**Note:** For event-sourced tables (not `entries` or `summaries`), if you write a repository method that does `UPDATE ... SET` without creating a new version, STOP and rewrite it.
...
|----|------|---|-------|------|
| #6795 | 4:36 PM | 🔵 | Project Has Empty Plugin Configuration | ~294 |
</claude-mem-context>