...
func (r *EntryRepository) Insert(ctx context.Context, entry domain.Entry) (int64, error) {
	...
	result, err := r.db.ExecContext(ctx, `
		INSERT INTO entries (type, content, parent_id, depth, location, scheduled_date, list_id, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
func (r *EntryRepository) GetByDate(ctx context.Context, date time.Time) ([]domain.Entry, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, type, content, parent_id, depth, location, scheduled_date, list_id, created_at
		FROM entries WHERE scheduled_date = ? AND list_id IS NULL
		ORDER BY created_at
	`, dateStr)
	if err != nil {
		return nil, err
...
func (r *EntryRepository) GetByDateRange(ctx context.Context, from, to time.Time) ([]domain.Entry, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, type, content, parent_id, depth, location, scheduled_date, list_id, created_at
		FROM entries WHERE scheduled_date >= ? AND scheduled_date <= ? AND list_id IS NULL
...
func (r *EntryRepository) scanEntry(row *sql.Row) (*domain.Entry, error) {
	...
	err := row.Scan(&entry.ID, &typeStr, &entry.Content, &parentID, &entry.Depth, &location, &scheduledDate, &listID, &createdAt)
...
func (r *EntryRepository) scanEntries(rows *sql.Rows) ([]domain.Entry, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&entry.ID, &typeStr, &entry.Content, &parentID, &entry.Depth, &location, &scheduledDate, &listID, &createdAt)
		if err != nil {
			return nil, err
		}
