...
### DeleteAll Methods - Import/Restore Only

The `DeleteAll` methods on all repositories intentionally use direct DELETE statements rather than creating DELETE markers. These methods are only used by `ImportService.clearAllData()` for full data restoration from backup files. In this context:
- Complete data replacement doesn't need incremental audit trails
- Imported data already contains its own history/versions
- Creating DELETE markers before import would be wasteful
...
## Issue 2: Comments in Production Code (100+ Violations)
...
## Issue 3: Mutable State Patterns Violate Functional Principles
...
## Issue 4: Nested If/Else Instead of Early Returns
...
## Issue 6: Service Layer Holds State (BackupService)
...
## Issue 7: TUI Complex Logic Not Extracted for Testing
...
## Issue 8: 12-Factor Output Stream Violations
